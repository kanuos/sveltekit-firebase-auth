...
export const actions: Actions = {
    ...
    "update-password": async ({ cookies, locals, request }) => {
        ...
        if (!validationReport.success) {
            ...
            return fail(400, { updatePassword: { success: false, message: validationReport.error.errors.map(el => el.message) } })
        }
        ...
        try {
            ...
            // firebase errors
            return fail(500, { updatePassword: { success: false, message: ["Something went wrong", error.message] } })
        }
    },
    add: async ({ locals, request }) => {
        // check if the logged in admin is a superuser or not
        if (!locals.user?.superuser) {
            return fail(409, { addAdmin: { success: false, message: ["User doesn't have superuser privileges"] } })
        }
        ...
        if (!validationReport.success) {
            return fail(400, { addAdmin: { success: false, message: validationReport.error.errors.map(el => el.message) } })
        }
    ...
    delete: async ({ locals, request }) => {
        if (!locals.user?.superuser) {
            return fail(409, { deleteAdmin: { success: false, message: ["User is not authorized to delete ADMINs"] } })
        }
        ...
        try {
            ...
            return { deleteAdmin: { success: true, message: [`ADMIN - (${uid}) has been deleted from the database`] } }
        } catch {
...
export const load: PageServerLoad = async () => {
    ...
    return {
        adminList: users.map(({ email, uid, customClaims, metadata }) => ({
            email: email as string, uid, superuser: customClaims?.superuser, date: metadata.creationTime
        } satisfies AdminType))
    }
};